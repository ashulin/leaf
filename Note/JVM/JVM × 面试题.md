...
![不同平台，不同 JVM](http://static2.iocoder.cn/5f64c6b9436f6efbe54cfc2d01727301)
...
![img](http://static2.iocoder.cn/images/JDK/2020_02_10/01.jpg)
...
![img](http://static2.iocoder.cn/images/JDK/2020_02_10/02.png)
...
![img](http://static2.iocoder.cn/images/JDK/2020_02_10/03.png)
...
JAVA 对象创建的过程，如下图所示：![JAVA 对象创建的过程](http://static2.iocoder.cn/images/JDK/2020_02_10/06.png)
...
![JAVA 对象创建的过程](http://static2.iocoder.cn/images/JDK/2020_02_10/07.png)
...
- 句柄定位：Java 堆会画出一块内存来作为句柄池，reference 中存储的就是对象的句柄地址，而句柄中包含了对象实例数据与类型数据各自的具体地址信息。![句柄定位](http://static2.iocoder.cn/images/JDK/2020_02_10/04.png)
- 直接指针访问：Java 堆对象的不居中就必须考虑如何放置访问类型数据的相关信息，而 reference 中存储的直接就是对象地址。 ![直接指针访问](http://static2.iocoder.cn/images/JDK/2020_02_10/05.png)
...
![标记-清除算法](http://static2.iocoder.cn/f07ac7c68994aa21b7b4271073ea4e5b)
...
![标记-整理算法](http://static2.iocoder.cn/52ea9a3813e09621c563b1cd4bdf0d22)
...
![复制算法](http://static2.iocoder.cn/cc3f228ca5353fd5deb4b22b7a9709e6)
...
![分代收集算法](http://static2.iocoder.cn/fec52af474f1250831d46b541e0fe7a9)
...
如下是一张对象创建时，分配内存的图：![内存分配](http://static2.iocoder.cn/bc22a30cbe93b8dedda080144a73b613)
...
![堆](http://static2.iocoder.cn/67796f9fc657b7b4dd0054f561248f5b)
...
![类加载器是如何加载 class 文件](http://static2.iocoder.cn/images/JDK/2020_02_10/08.png)
...
![双亲委派模型](http://static2.iocoder.cn/images/JDK/2020_02_10/09.png)
...
- 该模型要求除了顶层的 Bootstrap 启动类加载器外，其余的类加载器都应当有自己的父类加载器。子类加载器和父类加载器不是以继承（Inheritance）的关系来实现，而是通过组合（Composition）关系来复用父加载器的代码。简略代码如下：
    ...
    ```
    // java.lang.ClassLoaderpublic abstract class ClassLoader {    // ... 省略其它代码    // The parent class loader for delegation    // Note: VM hardcoded the offset of this field, thus all new fields    // must be added *after* it.    private final ClassLoader parent;}
    ```
...
```
// java.lang.ClassLoader// 删除部分无关代码protected Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {    synchronized (getClassLoadingLock(name)) {        // 首先，从缓存中获得 name 对应的类        Class<?> c = findLoadedClass(name);        if (c == null) { // 获得不到            try {                // 其次，如果父类非空，使用它去加载类                if (parent != null) {                    c = parent.loadClass(name, false);                // 其次，如果父类为空，使用 Bootstrap 去加载类                } else {                    c = findBootstrapClassOrNull(name);                }            } catch (ClassNotFoundException e) {            }            if (c == null) { // 还是加载不到                // 最差，使用自己去加载类                c = findClass(name);            }        }        // 如果要解析类，则进行解析        if (resolve) {            resolveClass(c);        }        return c;    }}
```
...
![脑图](http://static2.iocoder.cn/f20e86724ff310733d7556bd203dc061)