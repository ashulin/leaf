...
第一范式，强调**属性**的**原子性约束**，要求属性具有原子性，**不可再分解。**

> 举个例子，活动表（活动编码，活动名称，活动地址），假设这个场景中，活动地址可以细分为国家、省份、城市、市区、位置，那么就没有达到第一范式。

##### 第二范式（2NF）

第二范式，强调**记录**的**唯一性约束**，**表必须有一个主键**，并且没有包含在主键中的列**必须完全依赖于主键**，而**不能只依赖**于主键的**一部分。**
...
第三范式，强调**属性冗余性的约束**，即非主键列**必须直接依赖于主键。**
...
- InnoDB：默认引擎
- MyISAM：存储文件跨平台，独立于操作系统；
- MRG_MYISAM：几个相同MyISAM表的聚合器；一般用于服务器日志
- MEMORY：内存表，即存在系统内存中；
- CSV
- ARCHIVE：仅支持插入和查询；经常被用来当做仓库使用
- BLACKHOLE：插入的数据并不会保存，表永远保持为空，写入的任何数据都会消失
- PERFORMANCE_SCHEMA：MySQL用户不能该存储引擎的表
- FEDERATED：主要用于访问其它远程MySQL服务器一个代理
...
> CSDN zyz_1024:时间花费上：count(img) > count(del_flag) > count(name) > count(id) > count(1)=count(*) > count(status)=count(start_time)
...
- 1、对非常小的表，大部分情况下全表扫描效率更高，这种情况优化器会全表扫描；
...
- 应尽量避免在 `WHERE` 子句中对字段进行**函数**操作和**运算**，否则系统将可能无法正确使用索引。

- 多个单列索引并不是最佳选择，为了提高性能，可以使用复合索引。
...
![B-Tree 的结构](http://static2.iocoder.cn/84ea509fa091a10add4e7614e6cb37db)
...
![B+Tree 的结构](http://static2.iocoder.cn/259d196856a231aff5e3cf1505848af4)
...
再另外，可能有胖友有和艿艿的一样疑惑，在**辅助索引**如果相同的索引怎么存储？最终存储到 B+Tree 非子节点中时，它们对应的主键 ID 是不同的，所以妥妥的。如下图所示：![相同的索引怎么存储](http://static2.iocoder.cn/images/MySQL/2019_12_01/01.png)
...
![主键索引](http://static2.iocoder.cn/d49d260fc1eb8f992df0401b70d70e3d)
...
![辅助索引](http://static2.iocoder.cn/2fb922405a35479fa99eb2de4708638c)
...
2. **一致性** Consistency ：在事务开始之前和事务结束以后，数据库的完整性没有被破坏以及业务逻辑上的一致性。这表示写入的资料必须完全符合所有的预设[约束](https://zh.wikipedia.org/wiki/数据完整性)、[触发器](https://zh.wikipedia.org/wiki/触发器_(数据库))、[级联回滚](https://zh.wikipedia.org/w/index.php?title=级联回滚&action=edit&redlink=1)等。
...
- 1、脏读：（针对未提交数据）事务 A 读取了事务 B 更新的数据，然后 B 回滚操作，那么 A 读取到的数据是脏数据。
- 2、不可重复读：（针对其他提交前后，读取数据本身的对比）事务 A 多次读取同一数据，事务 B 在事务 A 多次读取的过程中，对数据作了更新并提交，导致事务 A 多次读取同一数据时，结果不一致。
- 3、幻读：（针对其他提交前后，读取数据本身的对比）系统管理员 A 将数据库中所有学生的成绩从具体分数改为 ABCDE 等级，但是系统管理员 B 就在这个时候插入了一条具体分数的记录，当系统管理员 A 改结束后发现还有一条记录没有改过来，就好像发生了幻觉一样，这就叫幻读。
...
- 共享锁（S锁）：不堵塞，多个用户可以同时读一个资源，互不干扰。
- 排他锁（X锁）：一个写锁会阻塞其他的读锁和写锁，这样可以只允许一个用户进行写入，防止其他用户读取正在写入的资源。
...
(8)     SELECT 
(9)     DISTINCT 
(6)		SUM(聚合函数) <select_list>
(1)     FROM <left_table>
(3)     <join_type> JOIN <right_table>
(2)     ON <join_condition>
(4)     WHERE <where_condition>
(5)     GROUP BY <group_by_list>#这里开始可以使用别名
...
```
# 首先查看主从库正在使用的binlog文件名称show master(slave) status# 删除之前一定要备份purge master logs before'2017-09-01 00:00:00'; # 删除指定时间前的日志purge master logs to'mysql-bin.000001'; # 删除指定的日志文件# 自动删除：通过设置binlog的过期时间让系统自动删除日志show variables like 'expire_logs_days'; # 查看过期时间set global expire_logs_days = 30; # 设置过期时间
```
